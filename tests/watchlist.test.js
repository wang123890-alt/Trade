...
await test('addWatchItem rejects blank stockId/stockName', async () => {
  const { watchItem, errors } = await addWatchItem({ stockId: '', stockName: '' });
  assert.equal(watchItem, null);
  assert.ok(errors.length > 0);
});

await test('addWatchItem adds a manual entry', async () => {
  const { watchItem, errors } = await addWatchItem({ stockId: '2330', stockName: '台積電' });
...
await test('addWatchItem records sold price/date when added from a sell flow', async () => {
...
await test('addWatchItem rejects a duplicate stockId', async () => {
  await addWatchItem({ stockId: '2330', stockName: '台積電' });
  const { watchItem, errors } = await addWatchItem({ stockId: '2330', stockName: '台積電' });
  assert.equal(watchItem, null);
...
await test('removeWatchItem deletes the entry', async () => {
  const { watchItem } = await addWatchItem({ stockId: '2330', stockName: '台積電' });
  await removeWatchItem(watchItem.id);
...
await test('autoAddFullyClosedFromTransactions adds a stock with no remaining position', async () => {
  const txs = [
  ...
  const added = await autoAddFullyClosedFromTransactions(txs);
...
await test('autoAddFullyClosedFromTransactions skips a stock that still has an open position', async () => {
  const txs = [
    tx({ id: 'b1', stockId: '2330', stockName: '台積電', type: 'BUY', dateTime: '2026-01-01', price: 100, quantity: 1000 }),
...
await test('autoAddFullyClosedFromTransactions skips a stock already in the watchlist', async () => {
  await addWatchItem({ stockId: '2330', stockName: '台積電' });
  const txs = [
    tx({ id: 'b1', stockId: '2330', stockName: '台積電', type: 'BUY', dateTime: '2026-01-01', price: 100, quantity: 1000 }),
    tx({ id: 's1', stockId: '2330', stockName: '台積電', type: 'SELL', dateTime: '2026-01-10', price: 120, quantity: 1000 }),
...
await test('autoAddFullyClosedFromTransactions uses the most recent SELL for sold price/date', async () => {
  const txs = [
    tx({ id: 'b1', stockId: '2330', stockName: '台積電', type: 'BUY', dateTime: '2026-01-01', price: 100, quantity: 1000 }),
