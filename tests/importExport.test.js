...
const { TransactionRepository, WatchlistRepository, initStore } = await import('../js/data/storage.js');
...
await test('buildExportPayload carries version, transactions, and watchlist', async () => {
  await TransactionRepository.save(tx('t1'));
  await WatchlistRepository.save(createWatchItem({ id: 'w1', stockId: '2317', stockName: '鴻海' }));
...
await test('validateImportPayload rejects a payload missing required fields', async () => {
...
await test('importFromPayload adds new transactions and skips ones already present', async () => {
  await TransactionRepository.save(tx('existing'));
  const payload = {
    version: '1.0',
...
await test('importFromPayload rejects a malformed payload without touching storage', async () => {
  await TransactionRepository.save(tx('existing'));
  const result = await importFromPayload({ version: '1.0' }); // missing transactions/watchlist arrays
  assert.equal(result.success, false);
  assert.ok(result.errors.length > 0);
  assert.equal(TransactionRepository.getAll().length, 1); // untouched
});

await test('importFromPayload warns (but does not block) when the merged history over-sells', async () => {
  const payload = {
    version: '1.0',
    transactions: [
