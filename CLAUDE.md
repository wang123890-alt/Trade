...
- **價格與K線都附註「來源 · 更新時間」**：持股頁市價旁邊、個股詳細頁K線標籤列旁邊，都會顯示類似「雅虎 · 14:32更新」的小字（`formatTime`，`js/utils/format.js`）。這是在「來源標籤」（見上面資料來源章節）之上再加一層——標籤只回答「這數字是不是收盤價」，時間戳記回答「這是不是剛剛才抓的」。都是**當次 session 記憶體內**（`holdings.view.js` 的 `priceMeta`、`stock-detail.view.js` 的 `renderChartFromBars` 參數），重新整理頁面就會消失，只有價格本身會持久化。
