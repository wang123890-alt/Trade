...
- **持股頁「AI解析」欄位與Excel匯出**：每檔持股卡片下方有一個可展開的「AI解析」文字欄位（`StockAiAnalysisRepository`，`js/data/storage.js`），單一可覆寫的自由文字欄，跟「看法紀錄」（`StockNotesRepository`，append-only 時間軸、使用者自己寫的）是分開的兩個概念，不要混用。「匯入AI解析」讓使用者一次貼上涵蓋多檔股票的AI分析文字，`classifyAiAnalysisText`（`js/core/aiAnalysisImport.js`）依「這一行提到哪支股票的代號或名稱」逐行分類、沒提到任何股票的行就沿用上一次提到的股票（多數AI寫法是先講一次標題再往下講好幾行不重複代號），完全比對不到任何持股的文字歸在「未比對到」區塊只顯示不寫入。分類結果一定先預覽（可個別勾選、可編輯每檔文字）才寫入，不會盲目自動覆蓋——寫入時如果該股已有舊內容，新內容用日期分隔線附加在後面而不是覆蓋掉。「匯出Excel」（`js/utils/exportExcel.js` 的 `buildExcelXml`/`downloadExcel`）用 SpreadsheetML（Excel 2003 XML格式，副檔名 `.xls`）純字串組出來，不需要額外套件或zip編碼，Excel可以直接開；欄位含代號/名稱/股數/成本均價/市價/未實現損益/損益%/AI解析。
